jQuery(function(){
    ...
    $.ajax({
        url:"/goods/findIndexGoodsClassList",
        type:"post",
        dataType:"json",
        success:function (data) {
            ...
            if(null!=data && data.length>0){
                ...
                for(var i=0;i<data.length;i++){
                    str+=" <div class=\"i_list\" id=\""+data[i].id+"\" style=\"cursor:pointer;top:0px;\" child_count=\""+data[i].goodClassList.length+"\">";
                    str+="<div class=\"li_nbv\" style=\"display: none;\"></div>";
                    str+="<h2><a href=\"#\">"+data[i].className+"</a></h2>";
                    str+="<p class=\"il\">";
                    if(data[i].goodClassList.length>0){
                        for(var j=0;j<data[i].goodClassList.length;j++){
                            var temp=data[i].goodClassList[j];
                            var br="";
                            if((j+1)%4==0){
                                br="<br/>";
                            }else{
...
function loadThreeGoodsList(id) {
    $.ajax({
        url:"/goods/loadThreeGoodsList/"+id,
        type:"post",
        dataType:"json",
        success:function (data) {
            if(null!=data && data.length>0){
